...
public:
	...
	// 형변환 연산자를 정의한다
	// 이 함수는 Complex 타입을 int 타입으로 형변환할 필요가 있을 때 호출한다
	// 반환 값의 타입을 명시하지 않았지만 int 타입으로의 형변환이므로 int 타입의 값을 반환해야 한다
	operator int()
	{
		// 실수부를 반환한다
		// 이 부분이 바로 Complex 타입에서 int 타입으로의 형변환 방법을 컴퓨터에게 제공하는 부분이라고 볼 수 있다
		return this->real;
	}

	// int 타입의 인자를 받는 생성자를 정의한다
	// int 타입을 Complex타입으로 형변환할 필요가 있을 때는 이 생성자가 호출된다
	// 인자 값을 사용해서 실수부의 값을 대입하고 허수부는 그냥 0을 대입한다
	// 이 부분이 바로 int 타입을 Complex 타입으로 형변환하는 방법을 컴퓨터에게 알려주는 부분이라고 볼 수 있다
	Complex(int i)
		:real(i),imaginary(0)
	{}
